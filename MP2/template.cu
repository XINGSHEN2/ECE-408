...
__global__ void matrixMultiply(float *A, float *B, float *C, int numARows,
  ...
  if ((numAColumns != numBRows) || (numCColumns != numBColumns) || (numCRows != numARows)){
    return;
  }
  // base on matrix A
  // int num_block_column = (numAColumns - 1) / blockDim.y + 1;  // how many blocks use on the column
  // int num_block_row = (numARows - 1) / blockDim.x + 1;  // how many blocks use on the row
  int Row = blockIdx.y * blockDim.y + threadIdx.y;
  int Col = blockIdx.x * blockDim.x + threadIdx.x;
  if ((Row < numARows) && (Col < numBColumns)){
    float Pvalue = 0;
    for (int k = 0; k < numAColumns; k++)
      Pvalue += A[Row * numAColumns + k] * B[k * numBColumns + Col]; // 矩阵乘法
...
int main(int argc, char **argv) {
  ...
  if (0 != (numCColumns % 16)){ DimGrid.x++; }
  if (0 != (numCRows % 16)){ DimGrid.y++; }
