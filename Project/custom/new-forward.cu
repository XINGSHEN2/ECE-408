...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    #define shared_3d(i2, i1, i0) shared[(i2) * (shared_width * shared_width) + (i1) * (shared_width) + i0]
    ...
    for(int c = 0; c < C; c++){
        ...
        for (int i = threadIdx.y; i < shared_width; i += TILE_WIDTH){
            for (int j = threadIdx.x; j < shared_width; j += TILE_WIDTH){
                ...
                if (x_in < W && y_in < H){
                    shared_3d(c, i, j) = in_4d(blockIdx.x, c, y_in, x_in);
                }
                else{
    ...
    if(h < H_out && w < W_out){
        ...
        for (int c = 0; c < C; c++){
            for (int p = 0; p < K; p++){
                for (int q = 0; q < K; q++){
                    acc += mask_4d(blockIdx.y, c, p, q) * shared_3d(c, threadIdx.y * S + p, threadIdx.x * S + q);
                }
            }
        }
        // out_4d(blockIdx.x, blockIdx.y, h, w) = acc;
        atomicAdd((&out_4d(blockIdx.x, blockIdx.y, h, w)), acc);
    }
